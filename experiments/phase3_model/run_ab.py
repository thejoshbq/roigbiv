#!/usr/bin/env python
"""Phase-3 Stage-1 model A/B driver (recall-refinement engagement).

Three arms, decomposing the cpsam-vs-CP3 swap so each pairwise contrast isolates
ONE variable (honors one_variable_per_experiment):

  cp3    deployed CP3 checkpoint, denoise_cyto3 ON   (stage1_backend=cellpose3, use_denoise=True)
  cp3nd  same CP3 checkpoint, denoise OFF            (stage1_backend=cellpose3, use_denoise=False)
  cpsam  Cellpose-SAM via sidecar (no denoise)       (stage1_backend=cpsam_sidecar)

Pairwise reads:
  cp3  vs cp3nd  -> denoise contribution        (model + input fixed)
  cp3nd vs cpsam -> pure architecture delta      (both on RAW mean_M)
  cp3  vs cpsam  -> as-deployed bottom line

All else identical: same source images (mean_M morph + vcorr_S), same diameter,
same cellprob/flow thresholds (the sidecar relays cfg.* verbatim).
...
Env knobs:
  P3_AB_MAX_FOVS    limit number of FOVs (smoke testing); default all
  P3_AB_ARMS        comma list to restrict arms; default cp3,cp3nd,cpsam
  P3_AB_MIN_FREE_GB abort a run if free disk below this; default 25
"""
...
OUT_ROOT = ROOT / "experiments/runs/phase3_model"
OUT_ROOT.mkdir(parents=True, exist_ok=True)
RESULTS = OUT_ROOT / "ab_results.json"
PROG = OUT_ROOT / "ab_progress.log"
FS = 7.5
MAX_FOVS = int(os.environ.get("P3_AB_MAX_FOVS", "0")) or None
MIN_FREE_GB = float(os.environ.get("P3_AB_MIN_FREE_GB", "25"))

# arm -> (stage1_backend, use_denoise)
ARM_CFG = {
    "cp3":   ("cellpose3", True),
    "cp3nd": ("cellpose3", False),
    "cpsam": ("cpsam_sidecar", False),
}
ARMS = [a for a in os.environ.get("P3_AB_ARMS", "cp3,cp3nd,cpsam").split(",") if a]
...
def run_arm(stem: str, movie: str, gt: str, arm: str, res: dict) -> None:
    ...
    if key in res and "error" not in res[key]:
        ...
        return
    backend, use_denoise = ARM_CFG[arm]
    ...
    cfg.stage1_backend = backend
    cfg.use_denoise = use_denoise
    cfg.no_viewer = True
    ...
    try:
        ...
        res[key] = {
            "stem": stem, "arm": arm, "runtime_s": round(dt, 1),
